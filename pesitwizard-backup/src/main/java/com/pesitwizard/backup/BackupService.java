...
public class BackupService {
                    ...
                    .forEach(
                            p -> {
                                try {
                                    ...
                                    if (Files.exists(m))
                                        i.setDescription(Files.readString(m).trim());
                                    list.add(i);
    ...
    public int cleanupOldBackups() {
        ...
        try (Stream<Path> s = Files.list(dir)) {
            ...
            for (int i = 0; i < all.size(); i++) {
                ...
                try {
                    if (Files.getLastModifiedTime(p).toInstant().isBefore(cutoff)) rm = true;
                } catch (IOException ignored) {
                }
                if (rm) {
                    try {
    ...
    private void restoreH2Backup(Path in) throws IOException {
        ...
        Path db = Path.of(path + ".mv.db");
        if (Files.exists(db))
            Files.copy(db, Path.of(db + ".bak." + LocalDateTime.now().format(TS)));
    ...
    private String extractH2Path() {
        ...
        String p =
                url.contains("h2:file:")
                        ? url.substring(url.indexOf("h2:file:") + 8)
    ...
    private DbInfo parsePostgresUrl() {
        ...
        return new DbInfo(
                p[0],
                p.length > 1 ? p[1] : "5432",
                p.length > 2 ? p[2].split("\\?")[0] : "postgres");
    }
    ...
    private static SecretKey deriveEncryptionKey(String masterKey) {
        ...
        try {
            ...
            PBEKeySpec spec =
                    new PBEKeySpec(
                            masterKey.toCharArray(), BACKUP_SALT, PBKDF2_ITERATIONS, KEY_LENGTH);
            try {
    ...
    private void encryptFile(Path input, Path output)
    ...
    private void decryptFile(Path input, Path output)
            throws IOException, java.security.GeneralSecurityException {
    ...
    private static boolean isBackupFile(String path) {
        return path.endsWith(".zip")
                || path.endsWith(".dump")
                || path.endsWith(".zip" + ENCRYPTED_SUFFIX)
                || path.endsWith(".dump" + ENCRYPTED_SUFFIX);
    }
