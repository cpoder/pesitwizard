...
/**
 * Integration test for sending large files with sync points and restart capability. Tests the 344MB
 * video file with periodic sync points.
 *
 * <p>File organization: Sequential (0) - appropriate for binary files like .mp4 Per PeSIT spec
 * 1.1.1.124.x: - 0 = sequential: data accessed sequentially - 1 = relative: access by record number
 * - 2 = indexed: access by key
 *
 * <p>Requires Connect:Express running on localhost:5100 Run with: mvn test
 * -Dtest=SendLargeFileWithRestartTest -Dpesit.integration.enabled=true
 */
...
public class SendLargeFileWithRestartTest {
    ...
    private static final String SMALL_FILE_PATH =
            "/home/cpo/pesit-data/client/2022-05-19 22-11-54.mkv";
    ...
    void setUp() {
        ...
        Assumptions.assumeTrue(
                testFile.exists() && testFile.canRead(), "Test file not found: " + LARGE_FILE_PATH);
    }
        ...
        try (PesitSession session =
            ...
            if (pi7 != null && pi7.getValue() != null && pi7.getValue().length >= 2) {
                ...
                if (syncIntervalKb > 0 && syncIntervalKb < 65535) {
                    ...
                    System.out.println(
                            "  Sync interval: " + (syncIntervalKb == 0 ? "disabled" : "undefined"));
                }
            }
            ...
            if (pi32 != null && pi32.getValue() != null) {
                ...
                System.out.println(
                        "  Negotiated record length (PI_32): " + negotiatedRecordLength + " bytes");
            }
            ...
            if (pi25 != null && pi25.getValue() != null) {
                ...
                System.out.println(
                        "  Negotiated max entity size (PI_25): " + negotiatedEntitySize + " bytes");
            }
            ...
            System.out.println(
                    "  Using chunk size: " + chunkSize + " bytes (limited by sync interval)");
                    ...
                    if (syncIntervalBytes > 0 && bytesSinceLastSync >= syncIntervalBytes) {
                        ...
                        Fpdu synFpdu =
                                new Fpdu(FpduType.SYN)
                                        ...
                                        .withParameter(
                                                new ParameterValue(
                                                        PI_20_NUM_SYNC, syncPointNumber));
                        ...
                        System.out.printf(
                                "  Sync point %d at %.1f MB (%.1f MB/s)%n",
            ...
            System.out.printf(
                    "  ✓ Sent %.1f MB in %.1f seconds (%.1f MB/s), %d sync points%n",
            ...
            try (FileInputStream fis = new FileInputStream(testFile)) {
                ...
                while ((bytesRead = fis.read(buffer)) != -1 && totalSent < partialSize) {
                    ...
                    session.sendFpduWithData(
                            new Fpdu(FpduType.DTF).withIdDst(serverConnectionId), chunk);
                    totalSent += bytesRead;
                    bytesSinceLastSync += bytesRead;
                ...
                while ((bytesRead = fis.read(buffer)) != -1) {
                    ...
                    if (syncIntervalBytes > 0 && bytesSinceLastSync >= syncIntervalBytes) {
                        ...
                        if (syncPointNumber % 50 == 0) {
                            double mbSent = totalSent / 1024.0 / 1024.0;
                            System.out.printf(
                                    "  Sync point %d at %.1f MB%n", syncPointNumber, mbSent);
                        }
                    }
                }
            }
