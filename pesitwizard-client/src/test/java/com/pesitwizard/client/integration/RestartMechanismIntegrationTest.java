 ...
 *
 * <p>Tests cover: - Send (PUSH) restart with multiple interrupts - Receive (PULL) restart scenarios
 * - Error handling and edge cases - Large file restart scenarios - Varying chunk sizes and entity
 * sizes
 *
 * <p>Requires PeSIT server running on test host/port. Run with: mvn test
 * -Dtest=RestartMechanismIntegrationTest -Dpesit.integration.enabled=true
 */
...
public class RestartMechanismIntegrationTest {
    ...
    /**
     * Test 1: Send with restart from sync point after interrupt Simulates network failure during
     * send, then resume from last sync point
     */
    @Test
    @Order(1)
    ...
    void testSendRestartFromSyncPoint() throws Exception {
        ...
        long resumedBytes =
                resumeSendFromSyncPoint(testData, transferId, VFILE_RESTART, lastSyncNumber);
    ...
    /**
     * Test 2: Multiple interrupts and restarts Interrupt transfer multiple times and restart each
     * time
     */
    @Test
    @Order(2)
    ...
    void testReceiveRestartFromSyncPoint() throws Exception {
        ...
        System.out.println(
                "  Received "
                        + partial.bytesReceived
                        + " bytes, interrupted at sync "
    ...
    /** Test 4: Restart from sync point 0 (beginning) Edge case: restart from the very beginning */
    @Test
    @Order(4)
    ...
    /** Test 5: Large file restart scenario Tests restart mechanism with a larger file (5MB) */
    @Test
    @Order(5)
    ...
    /** Test 6: Varying chunk sizes with restart Tests restart with different entity sizes */
    @Test
    @Order(6)
    ...
    void testRestartWithVaryingChunkSizes() throws Exception {
        ...
        int sync1 =
                sendPartialWithInterruptAndChunkSize(
                        testData, transferId, VFILE_RESTART, 2, 250, 256);
        long resumed1 =
                resumeSendWithChunkSize(testData, transferId, VFILE_RESTART, sync1, 250, 256);
        ...
        int sync2 =
                sendPartialWithInterruptAndChunkSize(
                        testData, transferId, VFILE_RESTART, 2, 1018, 1024);
        long resumed2 =
                resumeSendWithChunkSize(testData, transferId, VFILE_RESTART, sync2, 1018, 1024);
    ...
    /**
     * Test 7: Restart with sync interval boundary conditions Tests restart exactly at sync interval
     * boundaries
     */
    @Test
    @Order(7)
    ...
    private int sendPartialWithInterrupt(byte[] data, int transferId, String vfile, int stopAtSync)
            throws Exception {
        return sendPartialWithInterruptAndChunkSize(
                data, transferId, vfile, stopAtSync, CHUNK_SIZE, MAX_ENTITY_SIZE);
    }

    /** Sends partial data with interruption using custom chunk size */
    private int sendPartialWithInterruptAndChunkSize(
            byte[] data,
            int transferId,
            String vfile,
            int stopAtSync,
        ...
        try (PesitSession session = new PesitSession(channel, false)) {
            ...
            FpduWriter writer =
                    new FpduWriter(session, serverConnId, maxEntitySize, recordLength, false);
            int syncNumber = 0;
    ...
    private long resumeSendFromSyncPoint(byte[] data, int transferId, String vfile, int restartSync)
            throws Exception {
        return resumeSendWithChunkSize(
                data, transferId, vfile, restartSync, CHUNK_SIZE, MAX_ENTITY_SIZE);
    }

    /** Resumes send with custom chunk size */
    private long resumeSendWithChunkSize(
            byte[] data,
            int transferId,
            String vfile,
            int restartSync,
        ...
        try (PesitSession session = new PesitSession(channel, false)) {
            ...
            long resumeOffset =
                    calculateResumeOffset(syncIntervalBytes, serverRestartPoint, recordLength);
    ...
    private int resumeAndInterruptAt(
            byte[] data, int transferId, String vfile, int restartSync, int stopAtSync)
            throws Exception {
        ...
        try (PesitSession session = new PesitSession(channel, false)) {
            ...
            // Calculate resume offset based on server's decision
            long resumeOffset =
                    calculateResumeOffset(syncIntervalBytes, serverRestartPoint, CHUNK_SIZE);
            FpduWriter writer =
                    new FpduWriter(session, serverConnId, MAX_ENTITY_SIZE, CHUNK_SIZE, false);
            int syncNumber = serverRestartPoint; // Use server's decision, not our request
            long bytesSinceSync = 0;
            long offset = resumeOffset;
    ...
    private ReceiveResult receivePartialWithInterrupt(String vfile, int stopAtSync)
            throws Exception {
        TcpTransportChannel channel = new TcpTransportChannel(TEST_HOST, TEST_PORT);
        channel.setReceiveTimeout(30000);
        ...
        try (PesitSession session = new PesitSession(channel, false)) {
            ...
            session.sendFpduWithAck(
                    new Fpdu(FpduType.SELECT)
                            ...
                            .withParameter(
                                    new ParameterValue(PI_25_TAILLE_MAX_ENTITE, MAX_ENTITY_SIZE)));
                ...
                if (type == FpduType.DTF
                    ...
                    session.sendFpdu(
                            new Fpdu(FpduType.ACK_SYN)
                                    ...
                                    .withParameter(new ParameterValue(PI_20_NUM_SYNC, syncNumber)));
                    System.out.println(
                            "  Sync " + syncNumber + " - received " + totalReceived + " bytes");
    ...
    private ReceiveResult resumeReceiveFromSyncPoint(String vfile, int restartSync)
            throws Exception {
        TcpTransportChannel channel = new TcpTransportChannel(TEST_HOST, TEST_PORT);
        channel.setReceiveTimeout(30000);
    ...
    private void cleanupSession(PesitSession session, int serverConnId)
