...
class ServerControllerTest {
    ...
    void getServer_notFound_shouldReturn404() throws Exception {
        mockMvc.perform(get("/api/v1/servers/nonexistent-id")).andExpect(status().isNotFound());
    }

    @Test
    void getServerByName_notFound_shouldReturn404() throws Exception {
        mockMvc.perform(get("/api/v1/servers/name/nonexistent")).andExpect(status().isNotFound());
    }

    @Test
    void getDefaultServer_noDefault_shouldReturn404() throws Exception {
        mockMvc.perform(get("/api/v1/servers/default")).andExpect(status().isNotFound());
    }
    ...
    void createAndManageServer() throws Exception {
        ...
        // Get by name
        mockMvc.perform(get("/api/v1/servers/name/" + serverName)).andExpect(status().isOk());
        ...
        mockMvc.perform(
                        put("/api/v1/servers/" + created.getId())
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    void testConnection_notFound_shouldReturn404() throws Exception {
        mockMvc.perform(post("/api/v1/servers/nonexistent/test")).andExpect(status().isNotFound());
    }
}
