...
public class ServerController {
    ...
    public List<PesitServerDto> getAllServers() {
        return serverService.getAllServers().stream().map(serverService::mapToDto).toList();
    }

    @GetMapping("/enabled")
    public List<PesitServerDto> getEnabledServers() {
        return serverService.getEnabledServers().stream().map(serverService::mapToDto).toList();
    }
    ...
    @PutMapping("/{id}")
    public PesitServerDto updateServer(
            @PathVariable String id, @Valid @RequestBody PesitServerDto dto) {
                ...
                .map(
                        server -> {
                            ...
                            return ResponseEntity.ok(
                                    new TestResult(result.success(), result.message()));
                        })
                .orElse(ResponseEntity.notFound().build());
    }
