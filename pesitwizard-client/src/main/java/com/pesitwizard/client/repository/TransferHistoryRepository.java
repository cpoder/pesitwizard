...
public interface TransferHistoryRepository extends JpaRepository<TransferHistory, String> {
    ...
    @Query(
            "SELECT COALESCE(SUM(h.bytesTransferred), 0) FROM TransferHistory h WHERE h.status = :status AND h.startedAt >= :since")
    Long sumBytesTransferredSince(
            @Param("status") TransferStatus status, @Param("since") Instant since);
    ...
    @Query(
            "SELECT h FROM TransferHistory h WHERE "
