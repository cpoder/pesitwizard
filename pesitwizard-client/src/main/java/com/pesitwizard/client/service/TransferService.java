...
/**
 * Service chapeau pour les transferts PeSIT. Délègue l'exécution aux services spécialisés (send,
 * receive, message).
 */
@Slf4j
@Service
...
public class TransferService {
    ...
    public TransferResponse receiveFile(TransferRequest request) {
        ...
        receiveService.receiveFileAsync(
                request,
                history.getId(),
                server,
                config,
    ...
    public TransferStats getStats() {
        Long bytesLast24h =
                historyRepository.sumBytesTransferredSince(
        ...
        return historyRepository
                ...
                .map(
                        history -> {
                            ...
                            // Cancellation event is published by transfer thread via
                            // TransferContext
                        ...
                        original -> {
                            TransferRequest request =
                                    TransferRequest.builder()
                                            ...
                                            .password(
                                                    partnerService.resolvePassword(
    ...
    public Optional<TransferResponse> resumeTransfer(String transferId) {
        return historyRepository
                ...
                .filter(
                        h ->
                                h.getStatus() == TransferStatus.FAILED
    ...
    private TransferConfig resolveConfig(String configNameOrId) {
        ...
        return configRepository
                .findByName(configNameOrId)
                .or(() -> configRepository.findById(configNameOrId))
                .orElseThrow(
                        () -> new IllegalArgumentException("Config not found: " + configNameOrId));
    }
    ...
    private TransferHistory createHistory(
            PesitServer server,
