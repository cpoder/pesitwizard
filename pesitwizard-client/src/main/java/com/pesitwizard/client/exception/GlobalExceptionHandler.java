...
/**
 * Global exception handler for the REST API. Provides consistent error responses without exposing
 * internal details.
 */
@Slf4j
...
public class GlobalExceptionHandler {
    ...
    /**
     * Handle all other unexpected exceptions. Logs the full stack trace but only returns a generic
    ...
    /**
     * Extract a user-friendly message from a DataIntegrityViolationException. Looks for common
     * constraint patterns.
     */
    private String extractConstraintViolationMessage(DataIntegrityViolationException ex) {
        ...
        if (message.contains("Unique index")
            ...
            java.util.regex.Pattern pattern =
                    java.util.regex.Pattern.compile("VALUES\\s*\\([^)]*'([^']+)'");
