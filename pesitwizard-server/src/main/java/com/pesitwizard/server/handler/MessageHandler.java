...
public class MessageHandler {
    ...
    public Fpdu handleMsg(SessionContext ctx, Fpdu fpdu) {
        ...
        log.info(
                "[{}] MSG received: file={}, message length={}",
                ctx.getSessionId(),
                filename,
    ...
    /** Handle MSGDM FPDU - start of segmented message Sets up context to receive MSGMM and MSGFM */
    public Fpdu handleMsgDm(SessionContext ctx, Fpdu fpdu) {
        ...
        log.info(
                "[{}] MSGDM received: file={}, first segment length={}",
                ctx.getSessionId(),
                filename,
    ...
    public Fpdu handleMsgMm(SessionContext ctx, Fpdu fpdu) {
        ...
        log.debug(
                "[{}] MSGMM received: total length now={}",
                ctx.getSessionId(),
                messageBuffer.length());
    ...
    public Fpdu handleMsgFm(SessionContext ctx, Fpdu fpdu) {
        ...
        log.info(
                "[{}] MSGFM received: file={}, total message length={}",
                ctx.getSessionId(),
                filename,
                fullMessage.length());
    ...
    public Fpdu handleMsgReceiving(SessionContext ctx, Fpdu fpdu) {
        return switch (fpdu.getFpduType()) {
            ...
            default -> {
                log.warn(
                        "[{}] Unexpected FPDU {} while receiving message",
                        ctx.getSessionId(),
                        fpdu.getFpduType());
