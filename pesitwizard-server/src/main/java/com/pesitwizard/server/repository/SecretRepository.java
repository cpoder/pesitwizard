...
public interface SecretRepository extends JpaRepository<SecretEntry, Long> {
    ...
    /** Find global secrets */
    @Query(
            "SELECT s FROM SecretEntry s WHERE s.scope = 'GLOBAL' AND s.active = true ORDER BY s.name")
    List<SecretEntry> findActiveGlobalSecrets();

    /** Find secrets for a partner */
    @Query(
            "SELECT s FROM SecretEntry s WHERE (s.scope = 'GLOBAL' OR (s.scope = 'PARTNER' AND s.partnerId = :partnerId)) AND s.active = true ORDER BY s.name")
    List<SecretEntry> findActiveSecretsForPartner(@Param("partnerId") String partnerId);

    /** Find secrets for a server */
    @Query(
            "SELECT s FROM SecretEntry s WHERE (s.scope = 'GLOBAL' OR (s.scope = 'SERVER' AND s.serverId = :serverId)) AND s.active = true ORDER BY s.name")
    ...
    @Query(
            "SELECT s FROM SecretEntry s WHERE s.active = true AND s.expiresAt IS NOT NULL AND s.expiresAt < CURRENT_TIMESTAMP")
