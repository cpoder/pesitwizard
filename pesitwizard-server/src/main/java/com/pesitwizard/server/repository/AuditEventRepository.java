...
public interface AuditEventRepository extends JpaRepository<AuditEvent, Long> {
    ...
    /** Find by event type */
    Page<AuditEvent> findByEventTypeOrderByTimestampDesc(
    ...
    /** Find failures */
    @Query(
            "SELECT a FROM AuditEvent a WHERE a.outcome IN ('FAILURE', 'DENIED', 'ERROR') ORDER BY a.timestamp DESC")
    Page<AuditEvent> findFailures(Pageable pageable);

    /** Find security events */
    @Query(
            "SELECT a FROM AuditEvent a WHERE a.category = 'SECURITY' OR a.outcome = 'DENIED' ORDER BY a.timestamp DESC")
    Page<AuditEvent> findSecurityEvents(Pageable pageable);

    /** Find authentication events for a user */
    @Query(
            "SELECT a FROM AuditEvent a WHERE a.category = 'AUTHENTICATION' AND a.username = :username ORDER BY a.timestamp DESC")
    ...
    @Query(
            "SELECT COUNT(a) FROM AuditEvent a WHERE a.category = :category AND a.timestamp >= :since")
    long countByCategorySince(
            @Param("category") AuditCategory category, @Param("since") Instant since);

    /** Count failures since */
    @Query(
            "SELECT COUNT(a) FROM AuditEvent a WHERE a.outcome IN ('FAILURE', 'DENIED', 'ERROR') AND a.timestamp >= :since")
    long countFailuresSince(@Param("since") Instant since);

    /** Get event counts by category */
    @Query(
            "SELECT a.category, COUNT(a) FROM AuditEvent a WHERE a.timestamp >= :since GROUP BY a.category")
    List<Object[]> countByCategories(@Param("since") Instant since);

    /** Get event counts by outcome */
    @Query(
            "SELECT a.outcome, COUNT(a) FROM AuditEvent a WHERE a.timestamp >= :since GROUP BY a.outcome")
