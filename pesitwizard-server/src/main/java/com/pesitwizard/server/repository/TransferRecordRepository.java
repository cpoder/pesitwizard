...
/**
 * Repository for transfer records. Provides queries for transfer history, monitoring, and retry
 * management.
 */
@Repository
public interface TransferRecordRepository extends JpaRepository<TransferRecord, Long> {
    ...
    Page<TransferRecord> findByDirectionOrderByStartedAtDesc(
            TransferDirection direction, Pageable pageable);

    /** Find active transfers (in progress) */
    @Query(
            "SELECT t FROM TransferRecord t WHERE t.status IN ('INITIATED', 'IN_PROGRESS', 'PAUSED') ORDER BY t.startedAt DESC")
    List<TransferRecord> findActiveTransfers();

    /** Find active transfers for a specific server */
    @Query(
            "SELECT t FROM TransferRecord t WHERE t.serverId = :serverId AND t.status IN ('INITIATED', 'IN_PROGRESS', 'PAUSED') ORDER BY t.startedAt DESC")
    List<TransferRecord> findActiveTransfersByServerId(@Param("serverId") String serverId);

    /** Find transfers that can be retried */
    @Query(
            "SELECT t FROM TransferRecord t WHERE t.resumable = true AND t.retryCount < t.maxRetries AND t.status IN ('FAILED', 'INTERRUPTED') ORDER BY t.updatedAt ASC")
    List<TransferRecord> findRetryableTransfers();

    /** Find transfers by filename pattern */
    @Query(
            "SELECT t FROM TransferRecord t WHERE t.filename LIKE :pattern ORDER BY t.startedAt DESC")
    ...
    @Query(
            "SELECT t FROM TransferRecord t WHERE t.startedAt BETWEEN :start AND :end ORDER BY t.startedAt DESC")
    Page<TransferRecord> findByTimeRange(
    ...
    @Query(
            "SELECT COUNT(t) FROM TransferRecord t WHERE t.status IN ('INITIATED', 'IN_PROGRESS', 'PAUSED')")
    long countActiveTransfers();

    /** Count active transfers for a server */
    @Query(
            "SELECT COUNT(t) FROM TransferRecord t WHERE t.serverId = :serverId AND t.status IN ('INITIATED', 'IN_PROGRESS', 'PAUSED')")
    long countActiveTransfersByServerId(@Param("serverId") String serverId);

    /** Get total bytes transferred */
    @Query(
            "SELECT COALESCE(SUM(t.bytesTransferred), 0) FROM TransferRecord t WHERE t.status = 'COMPLETED'")
    long getTotalBytesTransferred();

    /** Get total bytes transferred for a partner */
    @Query(
            "SELECT COALESCE(SUM(t.bytesTransferred), 0) FROM TransferRecord t WHERE t.partnerId = :partnerId AND t.status = 'COMPLETED'")
    ...
    @Query(
            "SELECT t.partnerId, COUNT(t), SUM(t.bytesTransferred) FROM TransferRecord t WHERE t.status = 'COMPLETED' GROUP BY t.partnerId ORDER BY COUNT(t) DESC")
    List<Object[]> getTransferStatsByPartner();

    /** Get daily transfer statistics */
    @Query(
            value =
                    "SELECT DATE(started_at) as day, COUNT(*) as count, SUM(bytes_transferred) as bytes "
    ...
    @Modifying
    @Query(
            "UPDATE TransferRecord t SET t.status = 'INTERRUPTED', t.updatedAt = :now, t.errorMessage = 'Server shutdown' WHERE t.nodeId = :nodeId AND t.status = 'IN_PROGRESS'")
