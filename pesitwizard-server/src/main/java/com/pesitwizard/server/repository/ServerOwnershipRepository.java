...
/**
 * Repository for database-backed server ownership records. Used to serialize concurrent ownership
 * acquisition across cluster nodes.
 */
@Repository
public interface ServerOwnershipRepository extends JpaRepository<ServerOwnershipRecord, String> {
    ...
    @Modifying
    @Query(
            "DELETE FROM ServerOwnershipRecord r WHERE r.serverId = :serverId AND r.ownerNode = :ownerNode")
    int deleteByServerIdAndOwnerNode(
            @Param("serverId") String serverId, @Param("ownerNode") String ownerNode);
