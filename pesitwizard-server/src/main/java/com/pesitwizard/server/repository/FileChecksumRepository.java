...
public interface FileChecksumRepository extends JpaRepository<FileChecksum, Long> {
    ...
    Optional<FileChecksum> findByChecksumHashAndAlgorithm(
            String checksumHash, HashAlgorithm algorithm);
    ...
    /** Find by status */
    Page<FileChecksum> findByStatusOrderByCreatedAtDesc(
            VerificationStatus status, Pageable pageable);

    /** Find duplicates (same hash, multiple entries) */
    @Query(
            "SELECT f FROM FileChecksum f WHERE f.checksumHash IN "
    ...
    /** Find by direction */
    Page<FileChecksum> findByDirectionOrderByCreatedAtDesc(
    ...
    @Query(
            "SELECT f FROM FileChecksum f WHERE f.partnerId = :partnerId "
    ...
    @Query(
            "SELECT f FROM FileChecksum f WHERE f.filename LIKE %:pattern% ORDER BY f.createdAt DESC")
