...
public interface CertificateStoreRepository extends JpaRepository<CertificateStore, Long> {
    ...
    List<CertificateStore> findByStoreTypeAndActiveOrderByNameAsc(
            StoreType storeType, Boolean active);
    ...
    /** Find by purpose */
    List<CertificateStore> findByPurposeAndActiveOrderByNameAsc(
            CertificatePurpose purpose, Boolean active);
    ...
    Optional<CertificateStore> findByPartnerIdAndStoreTypeAndActiveTrue(
            String partnerId, StoreType storeType);

    /** Find certificates expiring before a date */
    @Query(
            "SELECT c FROM CertificateStore c WHERE c.active = true AND c.expiresAt IS NOT NULL AND c.expiresAt < :date ORDER BY c.expiresAt ASC")
    List<CertificateStore> findExpiringBefore(@Param("date") Instant date);

    /** Find expired certificates */
    @Query(
            "SELECT c FROM CertificateStore c WHERE c.active = true AND c.expiresAt IS NOT NULL AND c.expiresAt < CURRENT_TIMESTAMP ORDER BY c.expiresAt ASC")
    ...
    @Query(
            "SELECT c FROM CertificateStore c WHERE c.storeType = 'KEYSTORE' AND c.purpose = 'SERVER' AND c.active = true ORDER BY c.isDefault DESC, c.name ASC")
    List<CertificateStore> findActiveServerKeystores();

    /** Find all active truststores */
    @Query(
            "SELECT c FROM CertificateStore c WHERE c.storeType = 'TRUSTSTORE' AND c.active = true ORDER BY c.isDefault DESC, c.name ASC")
