...
/**
 * Component for tracking transfers in the database. Integrates with PesitSessionHandler to persist
 * transfer records.
 */
@Slf4j
@Component
...
public class TransferTracker {
    ...
    public void trackTransferStart(
            SessionContext ctx,
            String serverId,
            String nodeId,
        ...
        try {
            ...
            log.error(
                    "[{}] Failed to track transfer start: {}", ctx.getSessionId(), e.getMessage());
    ...
    public void trackTransferComplete(SessionContext ctx) {
        ...
        try {
            ...
            log.debug(
                    "[{}] Transfer tracking completed: {} ({} bytes)",
                    ctx.getSessionId(),
                    transferId,
                    bytesTransferred);
            ...
            log.error(
                    "[{}] Failed to track transfer completion: {}",
                    ctx.getSessionId(),
                    e.getMessage());
        }
    }
    ...
    public void trackTransferFailed(SessionContext ctx, String errorCode, String errorMessage) {
        ...
        try {
            ...
            log.error(
                    "[{}] Failed to track transfer failure: {}",
                    ctx.getSessionId(),
                    e.getMessage());
        }
    }
    ...
    public void trackTransferCancelled(SessionContext ctx, String reason) {
        ...
        try {
            ...
            log.error(
                    "[{}] Failed to track transfer cancellation: {}",
                    ctx.getSessionId(),
                    e.getMessage());
        }
    }
    ...
    public void trackTransferInterrupted(SessionContext ctx, String reason) {
        ...
        try {
            ...
            log.error(
                    "[{}] Failed to track transfer interruption: {}",
                    ctx.getSessionId(),
                    e.getMessage());
        }
    }
    ...
    /**
     * Track authentication failure as a failed transfer. This creates a transfer record with FAILED
     * status for connections that fail authentication before any transfer operation starts.
     */
    public void trackAuthenticationFailure(
        ...
        try {
            ...
            log.error(
                    "[{}] Failed to track authentication failure: {}",
                    ctx.getSessionId(),
                    e.getMessage());
        }
    }
