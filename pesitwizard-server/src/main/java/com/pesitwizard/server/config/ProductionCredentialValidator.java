...
/**
 * Validates that default/insecure credentials have been changed before
 * the application is allowed to start in a production profile.
 *
 * <p>Production mode is detected when the "postgres" Spring profile is active
 * (indicating a real database rather than the H2 development default).
 *
 * <p>If any default passwords are still in use, this validator logs a FATAL
 * error for each violation and throws {@link IllegalStateException} to
 * prevent the application from starting with insecure defaults.
 *
...
public class ProductionCredentialValidator implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        log.info("Production profile detected — validating that default credentials have been changed");
        ...
        if (!violations.isEmpty()) {
            ...
            throw new IllegalStateException(
                    ...
                    + "Change the following before starting: " + violations);
        }
    ...
    private void validateBasicAuthPasswords(List<String> violations) {
        ...
        Map<String, SecurityProperties.UserEntry> users = securityProperties.getBasicAuth().getUsers();
        ...
        for (Map.Entry<String, SecurityProperties.UserEntry> entry : users.entrySet()) {
            ...
            if (user.getPassword() != null
                ...
                violations.add(
                        "pesit.security.basic-auth.users." + username
                        + ".password is still set to the default value \"" + user.getPassword()
    ...
    /**
     * Validates that the CA keystore and truststore passwords have been changed
     * from their insecure defaults.
     */
