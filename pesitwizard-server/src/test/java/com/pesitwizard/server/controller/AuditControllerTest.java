...
class AuditControllerTest {
    ...
    void shouldGetRecentEvents() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/audit/recent")).andExpect(status().isOk());
    }
    ...
    void shouldGetEventsByCategory() throws Exception {
        Page<AuditEvent> page = new PageImpl<>(List.of(testEvent));
        when(auditService.getEventsByCategory(any(AuditCategory.class), anyInt(), anyInt()))
                .thenReturn(page);

        mockMvc.perform(
                        get("/api/v1/audit/category/{category}", "TRANSFER")
    ...
    void shouldGetFailures() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/audit/failures")).andExpect(status().isOk());
    }
    ...
    void shouldGetSecurityEvents() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/audit/security")).andExpect(status().isOk());
    }
    ...
    void shouldGetTransferEvents() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/audit/transfers")).andExpect(status().isOk());
    }
    ...
    void shouldGetEventsForUser() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/v1/audit/user/{username}", "testuser")
