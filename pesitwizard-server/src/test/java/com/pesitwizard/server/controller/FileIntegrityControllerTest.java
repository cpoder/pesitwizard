...
class FileIntegrityControllerTest {
    ...
    @MockitoBean private FileIntegrityService integrityService;
    ...
    void getChecksumShouldReturn404WhenNotFound() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/integrity/999")).andExpect(status().isNotFound());
    }
    ...
    void getChecksumsByStatusShouldReturnFilteredResults() throws Exception {
        ...
        when(integrityService.getChecksumsByStatus(
                        eq(VerificationStatus.VERIFIED), anyInt(), anyInt()))
                .thenReturn(new PageImpl<>(List.of(checksum)));
    ...
    void verifyFileShouldReturnVerificationResult() throws Exception {
        VerificationResult result =
                new VerificationResult(true, "Verified", VerificationStatus.VERIFIED);
