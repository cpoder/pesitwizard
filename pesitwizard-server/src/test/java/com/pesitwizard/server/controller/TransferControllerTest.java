...
class TransferControllerTest {
    ...
    class ListAndSearchTests {
        ...
        void shouldSearchTransfers() throws Exception {
            Page<TransferRecord> page = new PageImpl<>(List.of(testTransfer));
            when(transferService.searchTransfers(
        ...
        void shouldGetActiveTransfersByServer() throws Exception {
            when(transferService.getActiveTransfersByServer("server-1"))
        ...
        void shouldGetTransfersByStatus() throws Exception {
            Page<TransferRecord> page = new PageImpl<>(List.of(testTransfer));
            when(transferService.getTransfersByStatus(
                            eq(TransferStatus.IN_PROGRESS), anyInt(), anyInt()))
                    .thenReturn(page);

            mockMvc.perform(get("/api/v1/transfers/status/IN_PROGRESS")).andExpect(status().isOk());
        }
        ...
        void shouldGetTransfersByPartner() throws Exception {
            Page<TransferRecord> page = new PageImpl<>(List.of(testTransfer));
            when(transferService.getTransfersByPartner(eq("partner-1"), anyInt(), anyInt()))
                    .thenReturn(page);

            mockMvc.perform(get("/api/v1/transfers/partner/partner-1")).andExpect(status().isOk());
        }
        ...
        void shouldGetRetryableTransfers() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/transfers/retryable")).andExpect(status().isOk());
        }
    }
    ...
    class SingleTransferTests {
        ...
        void shouldReturn404ForNonExistent() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/transfers/non-existent")).andExpect(status().isNotFound());
        }

        @Test
        @DisplayName("should get transfers by session")
        void shouldGetTransfersBySession() throws Exception {
            when(transferService.getTransfersBySession("session-1"))
                    .thenReturn(List.of(testTransfer));

            mockMvc.perform(get("/api/v1/transfers/session/session-1")).andExpect(status().isOk());
        }
    ...
    class TransferActionsTests {
        ...
        void shouldCancelTransfer() throws Exception {
            ...
            when(transferService.cancelTransfer(eq("transfer-123"), anyString()))
    ...
    class StatisticsTests {
        ...
        void shouldGetDailyStatistics() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/transfers/stats/daily")).andExpect(status().isOk());
        }
    }
    ...
    class AdminTests {
        ...
        void shouldTriggerCleanup() throws Exception {
            ...
            mockMvc.perform(delete("/api/v1/transfers/cleanup")).andExpect(status().isOk());
