...
class PesitServerControllerTest {
    ...
    @MockitoBean private PesitServerManager serverManager;
    ...
    class CrudTests {
        ...
        void shouldReturn400ForInvalidConfig() throws Exception {
            when(serverManager.createServer(any()))
        ...
        void shouldUpdateServer() throws Exception {
            when(serverManager.updateServer(eq("server-1"), any(PesitServerConfig.class)))
    ...
    class RuntimeErrorTests {
        ...
        void shouldReturn500WhenStopFailsWithRuntimeError() throws Exception {
            doThrow(new RuntimeException("Failed to stop"))
                    .when(serverManager)
                    .stopServer("server-1");
