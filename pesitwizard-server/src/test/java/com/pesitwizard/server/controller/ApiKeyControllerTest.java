...
class ApiKeyControllerTest {
    ...
    void getApiKeyShouldReturn404WhenNotFound() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/apikeys/999")).andExpect(status().isNotFound());
    }
    ...
    void revokeApiKeyShouldRevokeExistingKey() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/apikeys/1/revoke")).andExpect(status().isOk());
    ...
    void revokeApiKeyShouldReturn404WhenNotFound() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/apikeys/999/revoke")).andExpect(status().isNotFound());
    }
    ...
    void regenerateApiKeyShouldReturn404WhenNotFound() throws Exception {
        when(apiKeyService.regenerateApiKey(999L))
                .thenThrow(new IllegalArgumentException("Not found"));

        mockMvc.perform(post("/api/v1/apikeys/999/regenerate")).andExpect(status().isNotFound());
    }
    ...
    void deleteApiKeyShouldDeleteExistingKey() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/apikeys/1")).andExpect(status().isNoContent());
    ...
    void deleteApiKeyShouldReturn404WhenNotFound() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/apikeys/999")).andExpect(status().isNotFound());
    }
