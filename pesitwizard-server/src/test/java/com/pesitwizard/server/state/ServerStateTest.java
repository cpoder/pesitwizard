...
class ServerStateTest {
    ...
    class TransitionValidationTests {
        ...
        void cn01ShouldOnlyAllowTransitionToConnect() {
            assertThat(ServerState.CN01_REPOS.canTransitionTo(ServerState.CN02B_CONNECT_PENDING))
                    .isTrue();
            assertThat(ServerState.CN01_REPOS.canTransitionTo(ServerState.CN03_CONNECTED)).isTrue();
            assertThat(ServerState.CN01_REPOS.canTransitionTo(ServerState.SF03_FILE_SELECTED))
                    .isFalse();
        }
        ...
        void cn02bShouldAllowValidTransitions() {
            assertThat(
                            ServerState.CN02B_CONNECT_PENDING.canTransitionTo(
                                    ServerState.CN03_CONNECTED))
                    .isTrue();
            assertThat(ServerState.CN02B_CONNECT_PENDING.canTransitionTo(ServerState.CN01_REPOS))
                    .isTrue();
            assertThat(ServerState.CN02B_CONNECT_PENDING.canTransitionTo(ServerState.ERROR))
                    .isTrue();
            assertThat(
                            ServerState.CN02B_CONNECT_PENDING.canTransitionTo(
                                    ServerState.SF03_FILE_SELECTED))
                    .isFalse();
        }
        ...
        void cn03ShouldAllowFileOperationsOrRelease() {
            assertThat(ServerState.CN03_CONNECTED.canTransitionTo(ServerState.SF01B_CREATE_PENDING))
                    .isTrue();
            assertThat(ServerState.CN03_CONNECTED.canTransitionTo(ServerState.SF02B_SELECT_PENDING))
                    .isTrue();
            assertThat(
                            ServerState.CN03_CONNECTED.canTransitionTo(
                                    ServerState.CN04B_RELEASE_PENDING))
                    .isTrue();
        ...
        void of02ShouldAllowTransferOrClose() {
            assertThat(
                            ServerState.OF02_TRANSFER_READY.canTransitionTo(
                                    ServerState.TDE01B_WRITE_PENDING))
                    .isTrue();
            assertThat(
                            ServerState.OF02_TRANSFER_READY.canTransitionTo(
                                    ServerState.TDL01B_READ_PENDING))
                    .isTrue();
            assertThat(
                            ServerState.OF02_TRANSFER_READY.canTransitionTo(
                                    ServerState.OF03B_CLOSE_PENDING))
                    .isTrue();
        }
        ...
        void tde02bShouldAllowDataReceptionTransitions() {
            assertThat(
                            ServerState.TDE02B_RECEIVING_DATA.canTransitionTo(
                                    ServerState.TDE02B_RECEIVING_DATA))
                    .isTrue();
            assertThat(
                            ServerState.TDE02B_RECEIVING_DATA.canTransitionTo(
                                    ServerState.TDE07_WRITE_END))
                    .isTrue();
            assertThat(
                            ServerState.TDE02B_RECEIVING_DATA.canTransitionTo(
                                    ServerState.TDE03_RESYNC_PENDING))
                    .isTrue();
        }
        ...
        void tdl02bShouldAllowDataSendingTransitions() {
            assertThat(
                            ServerState.TDL02B_SENDING_DATA.canTransitionTo(
                                    ServerState.TDL02B_SENDING_DATA))
                    .isTrue();
            assertThat(ServerState.TDL02B_SENDING_DATA.canTransitionTo(ServerState.TDL07_READ_END))
                    .isTrue();
            assertThat(
                            ServerState.TDL02B_SENDING_DATA.canTransitionTo(
                                    ServerState.OF02_TRANSFER_READY))
                    .isTrue();
        }
