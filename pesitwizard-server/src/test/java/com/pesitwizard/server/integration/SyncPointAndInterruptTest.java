 ...
 *
 * <p>Requires a PeSIT server running on localhost:5000 Run with: mvn test
 * -Dtest=SyncPointAndInterruptTest -Dpesit.integration.enabled=true
 */
...
public class SyncPointAndInterruptTest {
    ...
    void testSyncPointsDuringTransfer() throws Exception {
        ...
        try (PesitSession session = new PesitSession(new TcpTransportChannel(HOST, PORT))) {
            ...
            // Send second chunk
            session.sendFpduWithData(
                    new Fpdu(FpduType.DTF).withIdDst(serverConnId), "Second chunk. ".getBytes());
            ...
            // Send third chunk
            session.sendFpduWithData(
                    new Fpdu(FpduType.DTF).withIdDst(serverConnId), "Third chunk.".getBytes());
    ...
    void testTransferRestart() throws Exception {
        ...
        try (PesitSession session = new PesitSession(new TcpTransportChannel(HOST, PORT))) {
            ...
            assertEquals(FpduType.ACK_WRITE, ackWrite.getFpduType());
            assertTrue(
                    ackWrite.hasParameter(PI_18_POINT_RELANCE),
                    "ACK_WRITE should have restart point");
