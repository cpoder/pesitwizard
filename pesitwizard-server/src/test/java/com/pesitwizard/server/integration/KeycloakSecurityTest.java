 ...
 *
 * <p>For full Keycloak integration testing: 1. Start Keycloak: cd src/test/resources/keycloak &&
 * docker-compose up -d 2. Run tests with profile: -Dspring.profiles.active=keycloak-test
 *
 * <p>Users configured in Keycloak: - admin/admin (ADMIN role) - operator/operator (OPERATOR role) -
 * user/user (USER role)
 */
@SpringBootTest
...
public class KeycloakSecurityTest {
    ...
    private static final org.slf4j.Logger log =
            org.slf4j.LoggerFactory.getLogger(KeycloakSecurityTest.class);
    ...
    class JwtRoleExtractionTests {
        private static final org.slf4j.Logger log =
                org.slf4j.LoggerFactory.getLogger(JwtRoleExtractionTests.class);
        ...
        void shouldExtractRolesFromKeycloakRealmAccess() {
            ...
            assertThat(authorities).extracting("authority").contains("ROLE_OPERATOR");
    ...
    class ApiKeyAuthenticationTests {
        private static final org.slf4j.Logger log =
                org.slf4j.LoggerFactory.getLogger(ApiKeyAuthenticationTests.class);
        ...
        void shouldAuthenticateWithValidApiKey() throws Exception {
            ...
            // Test authentication
            mockMvc.perform(get("/api/v1/transfers").header("X-API-Key", apiKey))
        ...
        void shouldRejectInvalidApiKey() throws Exception {
            mockMvc.perform(get("/api/v1/transfers").header("X-API-Key", "invalid-key"))
        ...
        void shouldAuthenticateWithApiKeyInQueryParam() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/transfers").param("api_key", result.getPlainKey()))
                    .andExpect(status().isOk());
    ...
    class RbacTests {
        ...
        void shouldAccessPublicEndpointWithoutAuth() throws Exception {
            ...
            log.info(
                    "Public endpoint /actuator/health: status={} (accessible without auth)",
                    status);
        }
    }
    ...
    class ApiKeyManagementTests {
        ...
        void shouldRegenerateApiKey() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/transfers").header("X-API-Key", oldKey))
                    .andExpect(status().isUnauthorized());

            // New key should work
            mockMvc.perform(get("/api/v1/transfers").header("X-API-Key", newKey))
                    .andExpect(status().isOk());
        ...
        void shouldEnforceApiKeyRoles() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/certificates").header("X-API-Key", apiKey))
                    .andExpect(status().isForbidden());
    ...
    class KeycloakIntegrationTests {
        ...
        void shouldValidateJwtFromKeycloak() {
            ...
            log.info(
                    "JWK Set URI: {}/realms/{}/protocol/openid-connect/certs", KEYCLOAK_URL, REALM);
        }
    }
}
