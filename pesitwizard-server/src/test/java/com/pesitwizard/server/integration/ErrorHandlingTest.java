...
/**
 * Integration tests for error handling scenarios. Tests that the server returns proper
 * RCONNECT/ABORT with correct diagnostic codes.
 *
 * <p>Requires a PeSIT server running on localhost:5000 Run with: mvn test -Dtest=ErrorHandlingTest
    ...
    void testWrongServerName() throws Exception {
        ...
        try (Socket socket = new Socket(HOST, PORT)) {
            ...
            assertEquals(
                    ...
                    "Expected RCONNECT for wrong server");
    ...
    void testUnknownPartner() throws Exception {
        ...
        try (Socket socket = new Socket(HOST, PORT)) {
            ...
            if (response.getFpduType() == FpduType.RCONNECT) {
                ...
                System.out.println(
                        "  ✓ Server rejected unknown partner with RCONNECT (strict mode)");
                System.out.println(
                        "    Diagnostic: 0x"
                                + String.format("%02X%02X%02X", diag[0], diag[1], diag[2]));
            } else {
    ...
    void testVersionMismatch() throws Exception {
        ...
        try (Socket socket = new Socket(HOST, PORT)) {
            ...
            assertEquals(
                    FpduType.RCONNECT,
                    response.getFpduType(),
                    "Expected RCONNECT for unsupported version");
            ...
            System.out.println(
                    "  ✓ Server correctly rejected with RCONNECT (Version Incompatible)");
            System.out.println(
                    "    Diagnostic: 0x"
                            + String.format("%02X%02X%02X", diag[0], diag[1], diag[2]));
        }
