...
CX_SUBJECT="/C=FR/ST=IDF/L=Paris/O=Connect Express/OU=Server/CN=cx-server"
VALIDITY_DAYS=365
KEY_SIZE=2048
PASSWORD="changeit"
...
# 4. Generate CX Server certificate (for future use)
...
openssl pkcs12 -export -in cx-server-cert.pem -inkey cx-server-key.pem \
    ...
    -password pass:$PASSWORD 2>/dev/null
echo "   CX keystore: cx-server-keystore.p12"

# 5. Create CA truststore
echo "5. Creating CA truststore..."
...
# 6. Cleanup CSR and extension files
rm -f *.csr *.cnf *.srl
...
echo "  PW Server: pw-server-keystore.p12"
echo "  PW Client: pw-client-keystore.p12"
echo "  CX Server: cx-server-keystore.p12"
echo "  Truststore: ca-truststore.p12"
echo ""
