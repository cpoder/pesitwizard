...
if [ "$SSL_ENABLED" = "true" ]; then
    # SSL-enabled partner for outbound connections to PW server
    $TOM_DIR/config/tom_prm PARTNER \
        "NAME=PWSERVER,PASSWD=,STATE=E,TYPE=O,PROT=3,SESSION=1,MAXSES=10,MAXSESIN=05,MAXSESOUT=05,LINK=T,HOST=,TCPADDR=$PW_SERVER_IP,TCPPORT=05001,DPCSID=PWSRV01,DPCPSW=,NRETRY=3,INTSESST=30,INTTRANST=60,NATURE=$SSL_NATURE,SSLPARM=$SSL_SSLPARM_CLIENT,MODE=REPLACE"
else
    $TOM_DIR/config/tom_prm PARTNER \
...
if [ "$SSL_ENABLED" = "true" ]; then
    # SSL-enabled partner for inbound connections from PW client
    $TOM_DIR/config/tom_prm PARTNER \
        "NAME=PWSRV01,PASSWD=,STATE=E,TYPE=O,PROT=3,SESSION=1,MAXSES=10,MAXSESIN=05,MAXSESOUT=05,LINK=T,HOST=,NRETRY=3,INTSESST=30,INTTRANST=60,NATURE=$SSL_NATURE,SSLPARM=$SSL_SSLPARM_SERVER,MODE=REPLACE"
else
    $TOM_DIR/config/tom_prm PARTNER \
