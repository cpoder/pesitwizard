...
if [ "$SSL_ENABLED" = "true" ]; then
    # Try SSL-enabled partner first, fall back to non-SSL if not supported
    if $TOM_DIR/config/tom_prm PARTNER \
        "NAME=PWSERVER,PASSWD=,STATE=E,TYPE=O,PROT=3,SESSION=1,MAXSES=10,MAXSESIN=05,MAXSESOUT=05,LINK=T,HOST=,TCPADDR=$PW_SERVER_IP,TCPPORT=05001,DPCSID=PWSRV01,DPCPSW=,NRETRY=3,INTSESST=30,INTTRANST=60,NATURE=$SSL_NATURE,SSLPARM=$SSL_SSLPARM_CLIENT,MODE=REPLACE" 2>/dev/null; then
        echo "   SSL partner configured successfully"
    else
        echo "   WARNING: SSL not supported in this CX version, falling back to non-SSL"
...
if [ "$SSL_ENABLED" = "true" ]; then
    # Try SSL-enabled partner first, fall back to non-SSL if not supported
    if $TOM_DIR/config/tom_prm PARTNER \
        "NAME=PWSRV01,PASSWD=,STATE=E,TYPE=O,PROT=3,SESSION=1,MAXSES=10,MAXSESIN=05,MAXSESOUT=05,LINK=T,HOST=,NRETRY=3,INTSESST=30,INTTRANST=60,NATURE=$SSL_NATURE,SSLPARM=$SSL_SSLPARM_SERVER,MODE=REPLACE" 2>/dev/null; then
        echo "   SSL partner configured successfully"
    else
        echo "   WARNING: SSL not supported in this CX version, falling back to non-SSL"
        $TOM_DIR/config/tom_prm PARTNER \
