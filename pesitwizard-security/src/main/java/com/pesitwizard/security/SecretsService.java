...
/**
 * Service for managing secrets throughout the application. Provides a unified API for
...
public class SecretsService {
    ...
    /**
     * Encrypt a sensitive value with contextual path for better Vault organization. Creates
     * human-readable paths like "registry/github/password" in Vault.
     *
     * @param plaintext The value to encrypt
     * @param entityType The entity type (e.g., "registry", "orchestrator", "partner", "cluster")
     ...
     */
    public String encryptForStorage(
            String plaintext, String entityType, String entityName, String fieldName) {
        if (plaintext == null || plaintext.isBlank()) {
            return plaintext;
        }
    ...
    /**
     * Store a secret in external provider (Vault). For AES mode, this is a no-op as secrets are
    ...
    /**
     * Derive a SecretKey for file at-rest encryption. Uses HMAC-based key derivation from the
     * master key.
     *
    ...
    /**
     * Store a secret directly in Vault with a specific key. Returns a vault reference to store in
     * the database.
     */
    ...
    public record SecretsProviderStatus(String providerType, boolean available, String message) {}
}
