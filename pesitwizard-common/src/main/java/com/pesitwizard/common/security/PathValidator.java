...
/**
 * Utility class for validating and sanitizing file paths. Provides protection against path
...
public final class PathValidator {
    ...
    /**
     * Pattern to detect path traversal attempts in the raw input. Matches: - "../" or "..\\" at any
     * position - Null bytes (common attack vector) - Excessive dots
     */
    private static final Pattern PATH_TRAVERSAL_PATTERN =
            Pattern.compile("(?:^|[/\\\\])\\.\\.(?:[/\\\\]|$)|\\.\\.\\.|\\x00");

    /**
     * Pattern for valid filename characters. Allows alphanumeric, dots, dashes, underscores, and
     * spaces. Rejects special characters that could be dangerous.
     */
    ...
    /**
     * Validates that a path is safe for use, without checking against a base directory. Use this
     * for paths that will be validated by other means (e.g., storage connectors).
     *
    ...
    public static String validateFilename(String filename) {
        ...
        java.nio.file.Path fileNamePath = Paths.get(filename).getFileName();
        if (fileNamePath == null) {
            throw new SecurityException("Filename resolves to an empty path");
        }
    ...
    /**
     * Checks if a filename contains only safe characters. This is a strict check that rejects
     * special characters.
     *
